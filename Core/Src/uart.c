...
{
    // Wait until the UART is ready to transmit another character
    while (!(u_stdout->ISR & USART_ISR_TXE));
...
char usart_getc(void)
{
    ...
    while (!(u_stdin->ISR & USART_ISR_RXNE));  // Wait until RXNE (RX not empty) bit is set

    // Reading USART_DR automatically clears the RXNE flag
    return ((U8) (u_stdin->RDR & 0xFF));
}

void usart_flush(void)
{
    // Wait until transmission on the UART is finished
    while (!(u_stdout->ISR & USART_ISR_TC));
    u_stdout->ISR &= ~USART_ISR_TC;
}
